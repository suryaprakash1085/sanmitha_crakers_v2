...
export const Footer = () => {
  ...
  const name = company.company_name || "Sanmitha Fireworks";
        ...
        <div className="container-festive section-pad !py-14 grid gap-10 md:grid-cols-2 lg:grid-cols-5 px-4 md:px-8">
          <div className="lg:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-3">
              <div className="w-9 h-9 rounded-full bg-festive grid place-items-center overflow-hidden">
                {company.logo ? (
                  <img src={company.logo} alt={name} className="w-full h-full object-cover" />
                ) : (
                  <img src="/favicon.ico" alt={name} className="w-full h-full object-cover" />
                )}
              </div>
            ...
            <p className="text-sm text-white/60">
              {company.description || "Lighting up your celebrations with premium quality crackers since 2010."}
            </p>
